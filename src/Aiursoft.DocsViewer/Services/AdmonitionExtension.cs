...
public static partial class AdmonitionPreprocessor
{
    ...
    {
        return PreprocessCore(markdown, static html => html);
    }

    /// <summary>
    /// Converts admonitions while protecting the trusted wrapper HTML behind unpredictable
    /// placeholders. This lets Markdig disable all user-authored HTML without escaping the
    /// structures generated by this preprocessor.
    /// </summary>
    public static AdmonitionPreprocessResult PreprocessWithPlaceholders(string markdown)
    {
        var replacements = new Dictionary<string, string>();
        var prefix = $"AIURSOFTADMONITION{Guid.NewGuid():N}";
        var index = 0;

        var processed = PreprocessCore(markdown, html =>
        {
        ...
        return new AdmonitionPreprocessResult(processed, replacements);
    }

    private static string PreprocessCore(string markdown, Func<string, string> protectHtml)
    {
        ...
        for (var i = 0; i < lines.Length; i++)
        {
            ...
            var openingHtml = $"<div class=\"admonition {bgClass} border-start border-4 {borderClass} rounded-2 p-3 my-3\">\n" +
                              $"<p class=\"admonition-title fw-semibold mb-2\">{emoji} {safeTitle}</p>\n" +
                              (isCollapsible ? "<details open><summary class=\"d-none\"></summary>\n" : string.Empty);
            var closingHtml = (isCollapsible ? "</details>\n" : string.Empty) + "</div>\n";
            var html = protectHtml(openingHtml) + $"\n{body}\n\n" + protectHtml(closingHtml);
...
public sealed record AdmonitionPreprocessResult(
    string Markdown,
    IReadOnlyDictionary<string, string> HtmlReplacements);
